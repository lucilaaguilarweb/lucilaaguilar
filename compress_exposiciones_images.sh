...
# Script to compress and resize images in exposiciones folder
# This will significantly reduce file sizes while maintaining good quality

echo "Starting image compression and resizing process for exposiciones..."
...
BACKUP_DIR="public/images/exposiciones-backup-$(date +%Y%m%d-%H%M%S)"
echo "Creating backup at: $BACKUP_DIR"
cp -r public/images/exposiciones "$BACKUP_DIR"
...
# Find all image files in the exposiciones directory and its subdirectories
find public/images/exposiciones -type f \( -name "*.jpg" -o -name "*.jpeg" -o -name "*.png" -o -name "*.tif" -o -name "*.tiff" \) | while read -r filepath; do
    TOTAL_FILES=$((TOTAL_FILES + 1))
    filename=$(basename "$filepath")
    dirpath=$(dirname "$filepath")
    exhibition_name=$(basename "$dirpath")
